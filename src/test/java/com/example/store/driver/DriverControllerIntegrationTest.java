package com.example.store.driver;
...
class DriverControllerIntegrationTest {
    ...
    @Test
    void when_OrderExistsAndGetOrderListByDriver_expect_ListOfOrdersReturned() throws Exception {
        ...
        Order order = new Order(0, 50, "Thanks for the delivery", orderStatus, savedCustomer, driver, savedProduct);

        orderRepository.save(order);

        mockMvc.perform(
                        get("http://localhost:8080/api/v1/driver/get/orders/" + savedDriver.getId())
                ...
                .andExpect(jsonPath("$[0].driver.email").value("test-driver@mail.com"))
                .andExpect(jsonPath("$[0].driver.firstName").value("test-firstname"))
                .andExpect(jsonPath("$[0].driver.lastName").value("test-lastname"));
